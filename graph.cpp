...
//    {
//        cout<<"node: "<<iter->first<<". Outputs:";
//        for(int k=0;k<iter->second.size();k++)
//            cout<<iter->second[k]->getId()<<",";
//        cout<<endl;
//    }
//   cout<<endl;
//#endif
...
//    write.open("OUT.aag",ios::out|ios::trunc);
    ...
    for(map<unsigned int,AND>::iterator it3=all_ANDS.begin();it3!=all_ANDS.end();it3++)
        write<<it3->second.getId()<<" "<<it3->second.getInputs()[0]->getId()+((int)it3->second.getInputPolarities()[0])<<" "<<it3->second.getInputs()[1]->getId()+((int)it3->second.getInputPolarities()[1])<<endl;
...
#else
    for(int counter=9984;counter<num_imgs;counter=counter+BITS_PACKAGE_SIZE)
#endif
        {
...
//        if(probs_it->second<= threshold)
        {
#if DEBUG >=0
            dump2<<"0->probes_it->first:"<<probs_it->first<<"(1- new_ths[this->all_depths[probs_it->first/2]]):"<<(1- new_ths[this->all_depths[probs_it->first/2]]);
            dump2<<"(1- new_ths[this->all_depths[probs_it->first/2]]):"<<(float)(1- new_ths[this->all_depths[probs_it->first/2]])<<endl;
...
//        if(probs_it->second>= 1-threshold)
        {
#if DEBUG >=0
            dump2<<"1->probes_it->first:"<<probs_it->first<<"new_ths[this->all_depths[probs_it->first/2]]:"<<new_ths[this->all_depths[probs_it->first/2]]<<"(new_ths[this->all_depths[probs_it->first/2]]):"<<(float)(new_ths[this->all_depths[probs_it->first/2]])<<endl;
