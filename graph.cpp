...
void graph::setANDsProbabilities(mnist& mnist_obj){
    ...
    cout<<"Setting ANDs probabilities by applying image to AIG recursively."<<endl;
...
void graph::propagateAndDeleteAll(mnist& mnist_obj) {
    ...
    info_file_name+=to_string(1-THRESHOLD);
    info_file_name+="_simplif_info.txt";
    ofstream simpl_info(info_file_name),dump1("dump1.txt"),dump2("dump2.txt"),dump3("dump3.txt");
    map<unsigned int,float>::iterator probs_it;
    vector<int> visits;
    visits.push_back(2);
    //Initializing all inputs to 2. 2 means the variable is not a constant
...
//  all_ANDS.find(12)->second.setSignal(0);
#endif
